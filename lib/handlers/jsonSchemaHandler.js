...
JsonSchemaHandler.prototype.postHandle = function (msg) {
  ...
  return Promise.resolve(msg)
    ...
    .then(function (response) {
      ...
      logger.instance.info('[%s] received a response', that.name, { body: response });
      return response;
    })
    // If failed
    .catch(function (err) {
      logger.instance.error('[%s] could not send the request, received an error %s', that.name, err);
