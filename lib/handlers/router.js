...
HandlerRouter.prototype.route = function(sender, msg) {
  ...
  return self.preRoute(sender, msg)
    .then(() => {
      logger.info('[HandlerRouter] routes a message with tag: ' + msg.fields.consumerTag + ' routing key: ' + topic);
      var wasFound = this.handlers.some(function(handler) {
        if (handler.match(topic)) {
          logger.info('[HandlerRouter] a handler was found: ' + handler.name);
      ...
      if (!!this.defaultHandler && !wasFound) {
        logger.info('[HandlerRouter] no suitable was found, using default');
        this.defaultHandler.handle(sender, msg);
        self.postRoute(msg, null, new Error('Not Found'));
      }
    })
    .catch(() => {
      logger.info('[HandlerRouter] handle declined by the router preRoute hook');
    })
};
