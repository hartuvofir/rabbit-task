  ...
  handlers.forEach(function(handler, i) {
    ...
    if (!(handler instanceof MsgHandler)) {
      ...
      if (!handler.name || !handler.pattern || !handler.handle) {
        ...
        newHandlerInstance.handle = handler.handle;
      }
    }
  });
 ...
 * @param msg
 */
HandlerRouter.prototype.route = function(msg) {
  var routingKey = msg.fields.routingKey;
  console.info('[HandlerRouter] routes a message with tag: ' + msg.fields.consumerTag + ' routing key: ' + routingKey);
  var wasFound = this.handlers.some(function(handler) {
    if (handler.match(routingKey)) {
  ...
  if (!!this.defaultHandler && !wasFound) {
    ...
    this.defaultHandler.handle(msg);
  }
};
