...
MsgHandler.prototype.reply = function(sender, msg, res, headers) {
  if (!!res && this.shouldReply(msg, res)) {
    logger.info('[MsgHandler] replies to a msg with tag: ' + msg.fields.consumerTag);
    try {
      var body;
      var replyQueue;
      var options;
  ...
  logger.info('[MsgHandler] acks a msg with tag: ', msg.fields.consumerTag);
  sender.ack(msg);
};
  ...
  return this.preHandle(msg).then(this.doHandle.bind(this)).then(this.postHandle.bind(this)).then(function(res) {
    ...
    if (!(err instanceof TemporaryError) || msg.properties.headers['x-attempts-left'] <= 1) {
      ...
      if (sender.nackExchange) {
        logger.error('[MsgHandler] app-nacks a msg with tag: ', msg.fields.consumerTag);
        sender.appNack(msg);
      } else {
        logger.error('[MsgHandler] nacks a msg with tag: ', msg.fields.consumerTag);
        sender.nack(msg);
      }
    }
...
MsgHandler.prototype.parseJson = function (msg) {
  ...
  logger.info('[MsgHandler] handles msg', {metadata: msgBody});
