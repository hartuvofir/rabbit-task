...
MsgHandler.prototype.reply = function(sender, msg, res, headers) {
  if (!!res && this.shouldReply(msg, res)) {
    logger.info('[MsgHandler] replies to a msg with tag: ' + JSON.stringify(msg.fields.consumerTag));
    try {
      var body;
      var replyQueue;
      var options;
      ...
      logger.error('[MsgHandler] an unexpected error has while attempting to reply to the msg with tag:' +
        msg.fields.consumerTag + '\nError was ' + e.stack || e.toString() + '\nresponse was: ' + res.messageBody || res);
      return Promise.reject(e);
    }
  }
  logger.info('[MsgHandler] acks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
  sender.ack(msg);
};
...
MsgHandler.prototype.handle = function(sender, msg) {
  ...
  return this.preHandle(msg).then(this.doHandle.bind(this)).then(this.postHandle.bind(this)).then(function(res) {
    ...
    logger.error('[MsgHandler] received the following error from handler: ' + err.stack || err);
    ...
    if (!(err instanceof TemporaryError) || msg.properties.headers['x-attempts-left'] <= 1) {
      ...
      if (sender.nackExchange) {
        logger.error('[MsgHandler] app-nacks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
        sender.appNack(msg);
      } else {
        logger.error('[MsgHandler] nacks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
        sender.nack(msg);
      }
    }
...
MsgHandler.prototype.parseJson = function (msg) {
  ...
  logger.info('[MsgHandler] handles msg: ' + msgBody);

  var parsedMessage;
  try {
    parsedMessage = JSON.parse(msgBody);
  } catch(e) {
    logger.error('[MsgHandler] handler expected a message in a JSON format, but received the following: \n ' + msgBody +
