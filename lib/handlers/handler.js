...
MsgHandler.prototype.handle = function(sender, msg) {
  ...
  return this.preHandle(msg).then(this.doHandle.bind(this)).then(this.postHandle.bind(this)).then(function(res) {
    ...
    logger.instance.error('[MsgHandler] received the following error from handler: ' + err.stack || err);
...
MsgHandler.prototype.parseJson = function (msg) {
  ...
  logger.instance.info('[MsgHandler] handles msg', {body: msgBody});

  var parsedMessage;
  try {
    parsedMessage = JSON.parse(msgBody);
  } catch(e) {
    logger.instance.error('[MsgHandler] handler expected a message in a JSON format, but received the following: \n ' + msgBody +
