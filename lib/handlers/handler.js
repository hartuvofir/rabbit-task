...
MsgHandler.prototype.reply = function(sender, msg, res, headers) {
  if (!!res && this.shouldReply(msg, res)) {
    console.info('[MsgHandler] replies to a msg with tag: ' + JSON.stringify(msg.fields.consumerTag));
    try {
      var body;
      var replyQueue;
      var options;
  ...
  console.info('[MsgHandler] acks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
  sender.ack(msg);
};
  ...
  return this.preHandle(msg).then(this.doHandle.bind(this)).then(this.postHandle.bind(this)).then(function(res) {
    ...
    if (!(err instanceof TemporaryError)) {
      ...
      if (sender.nackExchange) {
        console.error('[MsgHandler] app-nacks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
        sender.appNack(msg);
      } else {
        console.error('[MsgHandler] nacks a msg with tag: ', JSON.stringify(msg.fields.consumerTag));
        sender.nack(msg);
      }
    }
