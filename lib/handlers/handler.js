...
MsgHandler.prototype.reply = function(sender, msg, res, headers) {
  if (!!res && this.shouldReply(msg, res)) {
    ...
    try {
      ...
      logger.instance.error('[MsgHandler] an unexpected error has occured while attempting to reply to the msg with tag:' +
        msg.fields.consumerTag, e, res.messageBody || res);
...
MsgHandler.prototype.handle = function(sender, msg) {
  ...
  return this.preHandle(msg).then(this.doHandle.bind(this)).then(this.postHandle.bind(this)).then(function(res) {
    ...
    logger.instance.error('[MsgHandler] received an error from handler', err);
...
MsgHandler.prototype.parseJson = function (msg) {
  ...
  try {
    ...
    logger.instance.info('[MsgHandler] handles msg', {body: parsedMessage});
  } catch(e) {
    logger.instance.error(
      '[MsgHandler] handler expected a message in a JSON format, but received an error',
