 ...
 * @returns {{}}
 */
function buildOptionsFromMsg(msg, headers, fields) {
  headers = headers || {};
  fields = fields || {};
 ...
 * @param msg
 */
Sender.prototype.reply = function(msg, response) {
  console.info('[Sender] replies to a msg with tag: ' + msg.fields.consumerTag);
  var replyTo = msg.properties.replyTo;
  if (!!replyTo) {
    var options = buildOptionsFromMsg(msg);
    this.sendToQueue(replyTo, new Buffer(response), options);

    return Promise.resolve(true);
  } else {
    this.error(msg, 'No reply queue was defined');
    return Promise.resolve(true);
  }
};
